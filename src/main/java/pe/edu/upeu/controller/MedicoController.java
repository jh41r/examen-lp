...
public class MedicoController {
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> updateMedico(@PathVariable("id") Long id, @Valid @RequestBody Medico con){
		Optional<Medico> m = medicoService.read(id);
		if(m.isEmpty()) {
			return new ResponseEntity<>(medicoService.update(con), HttpStatus.OK);
		}else {
			return new ResponseEntity<>(HttpStatus.NO_CONTENT);
		}
	}	
